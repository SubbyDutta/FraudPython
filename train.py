...
BALANCE_EXEMPTION = 60000      
AVG_AMOUNT_EXEMPTION = 50000   
...
user_avg_txn = np.random.exponential(scale=25000, size=n_users) 
user_balance_avg = np.random.exponential(scale=80000, size=n_users) 
...
amount = np.random.exponential(scale=20000, size=N)  

large_indices = np.random.choice(N, size=int(N * 0.02), replace=False)
amount[large_indices] = amount[large_indices] + np.random.uniform(AMOUNT_THRESHOLD, AMOUNT_THRESHOLD * 5, size=large_indices.size)
...
rule_amount_gt_balance_low_bal = (amount > balance * 1.3) & (balance < 5_00)
...
if current_rate < min_rate:
    n_needed = int(min_rate * N - fraud.sum())
    if n_needed > 0:
        zeros_idx = np.where(fraud == 0)[0]
        choose_idx = np.random.choice(zeros_idx, min(n_needed, zeros_idx.size), replace=False)
        fraud[choose_idx] = 1
...
df['user_exempt_balance_or_avg'] = ((df['balance'] >= BALANCE_EXEMPTION) | (df['avg_amount'] >= AVG_AMOUNT_EXEMPTION)).astype(int)
...
print(f"Fraud proportion in dataset: {df['label'].mean():.4f} (N={len(df)})")
